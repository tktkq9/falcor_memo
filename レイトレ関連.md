...
create()（とその中で呼ばれているinit()）でRayGen（1個、複数作る実装にはなっているが）、Miss（N_ray個）、HitGroupのEntryPointGroupVars（N_ray * N_mash個）のEntryPointGroupVarsを作成、保持される  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbODQzNzY3MDU3LDEwODUyMTkyODYsLTE1ND
