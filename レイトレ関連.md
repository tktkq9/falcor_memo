...
そこで各SubTableType（RayGen、Miss、HitGroup）ごとにapplyVarsToTable()が呼ばれ、サブオブジェクトの作成と設定が行われる  
...
SubTableTypeに対応するエントリーポイントすべてに対し、サブオブジェクトの設定バッファーpRecordを取得し、applyRtProgramVars()でサブオブジェクトの作成と設定を行い、バージョン管理l用変数astObservedChangeEpochの更新を行う  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTYzNDQwNTc0NywxMDgyOTQ1NTQ5LDcyNT
