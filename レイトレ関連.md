...
そこからsurfaceSpreadAngleを計算（Ray Tracing Gems Cjaper 20の（32）式とは違うが（32）式の導出の考え方と似たような感じで計算している。Fig20.7とコードを参照）  
  - RayConeMode::Unifiedの場合は当たった三角形の辺ごとの曲率の内もっとも大きいものを選び、

##### TexLODHelpers.slang（レイトレ部分）
- computeScreenSpaceSurfaceSpreadAngle(float3 positionW, float3 normalW, float betaFactorK1 = 1.0, float betaFactorK2 = 0.0) : Ray Tracing Gems Cjaper 20の（32）式。GBufferRaster.3d.slangで
- 
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM2OTYzMTY0MiwxNzM3NTQwMDA5LDE3NT
