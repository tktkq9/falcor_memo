...
DXRではラスタライズと違って、シェーダー変数のハンドルはShaderTableに設定される（つまりここでいうpRecordに設定される）  
この処理を行うにあたり、実際はコマンドリストの挙動をしない疑似コマンドリストクラスRtVarsCmdListを使うことによって、applyProgramVarsCommon()（通常はルートシグネチャーと変数ハンドルをコマンドリストに設定する関数）でコマンドリストの代わりにpRecordにハンドルを設定するような実装となっている  

#### RtVarsContext
TODO : こいつの存在意義
コマンドリストRtVarsCmdListを生成するがこれがほぼ何もしない  
また、LowLevelContextDataのコンストラクタにnullptrのコマンドキューを渡している。これだとコマンドリスト実行ができないので何をやっているかなぞ  

DXR用の（ローカルルートシグネチャーと関連する変数設定用の）Context、CopyContextのサブクラス?  
RtProgramVars以外では使われていない（ローカルルートシグネチャー関連のコマンドリスト設定はRtProgramVarsでしか行われていないっぽいので）  
ここで行っていることはRtVarsCmdListを生成し保持することと、  
バリアー処理はこのContextではなくDeviceクラスの方のRenderContextに任せるようにすることのみ  

#### RtVarsCmdList
RtVarsContextでのみ使われる  
ほぼ何もしないコマンドリストのように見える  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTE0MzE1OTI1MiwtMjA2MzQxNDIyNCwtMT
M4MTc3NTAzNSwxMTI0NTEwMTE3LDk5OTI0NTIyMiwtMTc4Mjk4
MzAyOCw5MjgzOTc3MTEsLTEwOTU5MzUzNTMsLTE1NzY1ODMxNT
EsMTU1MDMzMDgzNiwxNjM0NDA1NzQ3LDcyNTI5NjQzMSwtMTc2
NzE0OTM2MCwtMTc2NzE0OTM2MCwxMzE2MTkyNzYwLC0yMjA2NT
Y2MzMsLTE3NjQyNTYyMzQsMTI3MTMwNjY0NywtODMwNjIzNTg2
LDEwODUyMTkyODZdfQ==
-->