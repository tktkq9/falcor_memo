...
- computeSpreadAngleFromCurvatureIso() : これがRayConeが更新されていく際の角度の計算として使われている。3通りの計算方法があるが、最後のFast Approximationが使われている。これはarcLength ≒ rayConeWidth / dn（平面上でのコーンの面積） = s として、面に対する法線角度の変異 φ = s * 曲率（= dφ/ds）から
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTUxODUzNzQsMTA0MDAwNjUyOCw1NjU0Mz
