...
この処理を行うにあたり、実際はコマンドリストの挙動をしない疑似コマンドリストクラスRtVarsCmdListを使うことによって、applyProgramVarsCommon()（通常はルートシグネチャーと変数ハンドルをコマンドリストに設定する関数）でpRecordにハンドルを設定するような実装となっている  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjAzMzAxNzk0MywtMjA2MzQxNDIyNCwtMT
