...
DXRではラスタライズと違って、シェーダー変数のハンドルはShaderTableに設定される（つまりここでいう）  
pRecordのハンドル設定はapplyProgramVarsCommon()の呼び出し（この関数は通常はルートシグネチャーと変数ハンドルをコマンドリストに設定する関数）で行われるが、通常のコマンドリストとRtVarsCmdList
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTg3NTQwMjM2NCwtMjA2MzQxNDIyNCwtMT
