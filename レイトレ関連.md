...
この処理を行うにあたり、実際はコマンドリストの挙動をしないRtVarsCmdListを使うことによって、applyProgramVarsCommon()の呼び出し（この関数は通常はルートシグネチャーと変数ハンドルをコマンドリストに設定する関数）でpRecordにハンドルを設定するような実装となっている  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE5NDE0MzE1MSwtMjA2MzQxNDIyNCwtMT
