...
- computeSpreadAngleFromCurvatureIso() : これがRayConeが更新されていく際の角度の計算として使われている。3通りの計算方法があるが、最後のFast Approximationが使われている。これはTEXLOD_SPREADANGLE_FROM_CURVATURE_MODE == 1での計算でarcLength ≒ rayConeWidth / dn（平面上でのコーンの面積） = s と近似し、面に対する法線角度の変異 φ = s * 曲率（= dφ/ds）からRayConeのβを計算している。ようするにFig20.7の亜種
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTE2NzA1NTUzNDksLTE0MzUzNzYyODgsLT
