...
シェーダー変数としては  
- nodes : mpBVHNodesBuffer = mNodes、LightBVHの本体  トップノードを0として、左ノードから順に格納されている。そのため子ノードの左側は自分のインデックス+1で計算できるので、リーフじゃないノードは右側のインデックスのみを持っている。  
リーフじゃないノードは
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjA1NTUwODAyMCwxNDU1MDQ4NzA1LC0xND
