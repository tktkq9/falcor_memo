...
一方updateNodeIndices()の方では、refit()で必要となるデータとして、ツリーの各depthの位置と数情報をまとめたmPerDepthRefitEntryInfoと、そのmPerDepthRefitEntryInfoでアクセスされる、nodes配列のノードインデックスを上から順に詰めていった配列であるmpNodeIndicesBufferを作成する（ただしリーフノードは問答無用で一番左にまとめてある。リーフノードのdepthが同じ保証はないので）  
...
その更新情報をもとに、に下から順にリーフじゃないノード更新をする  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMTAxODU3MzM1LDE0NTUwNDg3MDUsLTE0ND
k2MjUzMzYsMTk1MDUwNjg2MSwyMDc3NTk0MzEsMTIwMjMyNjQ5
MCwxNzM4OTIxMDY4LDQyMDIyNjQzNSwxNTU4Nzc0OTg4LDEzOT
M2NjEyMzQsLTc5MTMyMTcwNiwtMTkyMDczNDU1MiwxMTI3MDMx
MTY3LDEyMDYzNjQyNjMsMTI1NDA0NDY3NSwtMTcyNzUwMjU5Ny
w4ODU0MzE5NTQsNDA1MjE4NDQ5LC0xOTE2Njk2NDQ2LDE5NTc1
ODE0NDddfQ==
-->