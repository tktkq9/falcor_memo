...
- Equal : computeSplitWithEqual() : トライアングル数に対し等分割するSplitResultの計算  
3軸の内最も距離が長い軸で分割する。分割場所はtriangleRange.middle()  
- BinnedSAH: computeSplitWithBinnedSAH()、SAH or VHによるSplitResultの計算、式（15）  
- binAlongDimension各binに対応するトライアングルのAABBをもとに各binのトライアングル包括AABBを計算、  
そこから各分割箇所ごとのコストをevalSAH()により計算し、一番コストが低い分割位置
- BinnedSAOH: computeSplitWithBinnedSAOH() : SAOH or VOHにおるSplitResultの計算、式（16）  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTc2MDcxNzExOCwxOTUwNTA2ODYxLDIwNz
c1OTQzMSwxMjAyMzI2NDkwLDE3Mzg5MjEwNjgsNDIwMjI2NDM1
LDE1NTg3NzQ5ODgsMTM5MzY2MTIzNCwtNzkxMzIxNzA2LC0xOT
IwNzM0NTUyLDExMjcwMzExNjcsMTIwNjM2NDI2MywxMjU0MDQ0
Njc1LC0xNzI3NTAyNTk3LDg4NTQzMTk1NCw0MDUyMTg0NDksLT
E5MTY2OTY0NDYsMTk1NzU4MTQ0NywtMTE2ODgwMjcwLC0xNzMy
MDM2MTUzXX0=
-->