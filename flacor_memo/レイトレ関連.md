...
EmissiveUniformSamplerはlightCollectionをシェーダーに設定してそれを使っていたが、  
こちらはサンプラーのシェーダー要変数として、_lightBVH構造体の値を設定し、サンプル取得時に使う  
...
sampleLight() : "18.4.3 LIGHT IMPORTANCE SAMPLING" によりLightBVHからトライアングルを選択し、そのトライアングルの面の内の1点をサンプリングし、そのTriangleLightSample lsを求める関数  
トライアングル選択で1Dサンプル1つ、トライアングルの1点選択で2Dサンプル1つを消費する  
EmissiveLightSamplerHelpers.slangの時と同じく、トライアングルのpdfは正確なpdfではなく、式（12）の余分な係数がついていることに注意  
...
ちなみにAABB内にShading point（origin）が入っていた場合はθ_u = π  
  ...
  originからAABB8点へのベクトルの平均によるコーン方向を計算し、  
  そのコーン方向とoriginからAABB8点へのベクトルとの角度の内最も大きいものを返す  
  このようなコーン方向の出し方をしているが、boundCosineTerm()ではorigin - centerでθ_iを計算しているので、この計算方法の場合コーン方向が一致していないことに気を付ける  
    - BoxToCenter : boundBoxSubtendedConeAngleCenter()呼び出し  
  3つの方法の中で一番最小のθ_uを計算する  
  高速化のため計算がちょっと分かりづらいが、やっていることはoriginから最も近いAABBの4点の内最も角度が大きいものを返す  
  - pickTriangle() : 
...
- computeLightingCone() : 
- computeLightingConesInternal() : buildInternal()完了の後に
a
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTIwMzcwMzEwNjAsMTI1NDA0NDY3NSwtMT
cyNzUwMjU5Nyw4ODU0MzE5NTQsNDA1MjE4NDQ5LC0xOTE2Njk2
NDQ2LDE5NTc1ODE0NDcsLTExNjg4MDI3MCwtMTczMjAzNjE1My
w4NzI3NDI0MywxMDAyMTcyMTQ2LC0zNTY4NjMzMTMsMTAxNTA5
MDQwMCwzNDg5NTAyMDIsMjE4MTMyOTAyLC0yODI4NzkxMzIsLT
k5MjIwNDAzLC0xNTkxNjAwNzAzLC0xODcwNDgzMzAyLDk5NjU5
MDkyNl19
-->