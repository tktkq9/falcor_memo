...
シェーダー変数としては  
- nodes : mpBVHNodesBuffer = mNodes、トップノードを0として、左ノードから順に格納されている。そのため子ノードの左側は自分のインデックス+1で計算できるので、リーフじゃないノードは右側のインデックスのみを持っている  
- triangleIndices : mpTriangleIndicesBuffer、左側のリーフノードから順にリーフノード内のとらいあんぐ


この変数の作成自体はLightBVHBuildernによって行われ、  
mpLightCollectionからmNodes、mpTriangleIndicesBufferのデータ、mpTriangleBitmasksBufferのデータが作られ、  
uploadCPUBuffers()でバッファーに送られ、  
残りのバッファーであるmpNodeIndicesBufferはそれらのデータからupdateNodeIndices()によって作られる  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTUzODY3MTg4NywxNDU1MDQ4NzA1LC0xND
