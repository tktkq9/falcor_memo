...
このLightBVHの作成自体はLightBVHBuildernによって行われ、  
mpLightCollectionからmNodes、mpTriangleIndicesBufferのデータ、mpTriangleBitmasksBufferのデータが作られ、  
uploadCPUBuffers()でバッファーに送られ、  
一方updateNodeIndices()の方では、refit()で必要となるデータとして、ツリーの各depthの位置と数情報をまとめたmPerDepthRefitEntryInfoと、そのmPerDepthRefitEntryInfoに対応するノードインデックス配列mpNodeIndicesBufferを作成する

シェーダー変数割り当てとしての機能だけではなく、シーンのトライアングルの位置などが変更した際のrefit()処理も担当する  
これはGPU側で行われ、LightBVHRefit.cs.slangのシェーダー処理を用いる  
ここで、updateNodeIndices()によって作成されたmpNodeIndicesBufferが使われ、  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTEyODI0MjExNDcsMTQ1NTA0ODcwNSwtMT
Q0OTYyNTMzNiwxOTUwNTA2ODYxLDIwNzc1OTQzMSwxMjAyMzI2
NDkwLDE3Mzg5MjEwNjgsNDIwMjI2NDM1LDE1NTg3NzQ5ODgsMT
M5MzY2MTIzNCwtNzkxMzIxNzA2LC0xOTIwNzM0NTUyLDExMjcw
MzExNjcsMTIwNjM2NDI2MywxMjU0MDQ0Njc1LC0xNzI3NTAyNT
k3LDg4NTQzMTk1NCw0MDUyMTg0NDksLTE5MTY2OTY0NDYsMTk1
NzU4MTQ0N119
-->