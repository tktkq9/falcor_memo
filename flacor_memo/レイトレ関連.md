...
- evalBVHTraversalPdf() : bitmaskによってトラバースしつつcomputeImportance()でpdfを計算し、  
そのpdfとリーフノードインデックスを返す  
sampleLightViaBVH()のpdf計算部分だけ
- evalNodeSamplingPdf() : _USE_UNIFORM_TRIANGLE_SAMPLINGの場合は一様サンプリングのpdf、  
そうじゃない場合は
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbLTM2Mzg0NzMxOSwxNzM4OTIxMDY4LDQyMD
