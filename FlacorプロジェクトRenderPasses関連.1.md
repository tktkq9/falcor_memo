...
そのバッファーとハンドルの対応付けの際、現在の変数構造情報の更新とそのバッファーサイズ取得をupdateSpecialization()でSlangAPIを用いて行い、バッファーがない（かサイズが足りていない）場合はバッファーの（再）作成も行われる  
...
<!--stackedit_data:
eyJoaXN0b3J5IjpbMjY4NDc3MDAxLC0xMDE1NTY2MDUyLDE2NT
